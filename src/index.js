const defaultInitialStateMerge = (a, b) => ({...a, ...b});

export const makeDuck = (options) => {

  const {initialStateMerge} = options || {};

  const iStateMerge = initialStateMerge || defaultInitialStateMerge;

  const mergeDucks = (...args) =>
    args.reduce((a = {}, b = {}) => {
      // Directly merge initialState and actions
      const initialState = iStateMerge(a.initialState, b.initialState);
  ...
  return function Ducky(...args) {
    const ns = args[0];

    // ns is optional, if missing recure with default ns ''
    if (typeof ns !== 'string') {
      return Ducky.apply(this, [''].concat(args));
    }
